...
GOTEST_PRIV_TAGS_FLAGS := $(GO_TAGS_FLAGS) privileged_tests
...
tests-privileged:
	...
	for pkg in $(patsubst %,github.com/cilium/cilium/%,$(PRIV_TEST_PKGS)); do \
		PATH=$(PATH):$(ROOT_DIR)/bpf $(GO_TEST) $(TEST_LDFLAGS) $$pkg $(GOTEST_UNIT_BASE) -tags=$(call join-with-comma,$(GOTEST_PRIV_TAGS_FLAGS)) $(GOTEST_COVER_OPTS) \
		|| exit 1; \
...
bench-privileged:
	...
	$(QUIET)for pkg in $(patsubst %,github.com/cilium/cilium/%,$(TESTPKGS)); do \
		$(GO_TEST) $(TEST_UNITTEST_LDFLAGS) $(GOTEST_BASE) $(BENCHFLAGS) \
			-tags=$(call join-with-comma,$(GOTEST_PRIV_TAGS_FLAGS)) $$pkg \
		|| exit 1; \
	done
