...
func runOperator() {
	...
	// for resourcelock config else fallback to non HA mode for Operator.
	// We only support operator in HA mode for Kubernetes Versions having support for
	// LeasesResourceLock
	// LeasesResourceLock type is preferred for resourcelocks because edits to Leases are less common
